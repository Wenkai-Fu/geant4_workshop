...
void RunAction::EndOfRunAction(const G4Run* run)
{
	...
	if (IsMaster()) {
		...
		if (nofEvents == 0)
			...
			G4cout << "\nTotal number of events = " << nofEvents << G4endl;

		G4cout << "\nrerg, rerg2 = " << verg << "  " << verg2 << G4endl;

		vec_double ans = cal_ave(verg, verg2, nofEvents);

		G4cout << G4endl
		<< "Ave. total deposited energy in tank, relative error = "
...
vec_double RunAction::cal_ave(G4double var, G4double var2, G4int n){
	 ...
	 * Parameters:
	 *      var: sum of a variable
	 *     var2: sum of variable ^2
